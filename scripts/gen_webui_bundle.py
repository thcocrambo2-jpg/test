...
-------------------------------
Nuitka follows imports. A module named by
`--include-module=ember.web.webui_bundle` comes along whole, while a data
file needs its own `--include-data-files` entry, a correct relative
destination, and a runtime path that resolves inside a onefile extraction.
...
request asks for it — and `ember/web/spa.py` mostly hands the gzip stream
to the browser untouched, so usually not even then.
