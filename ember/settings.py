...
Every environment variable the app reads, in one place:
    ...
    KREA2_UI_REQUIRE_TOKEN      1 demands the UI token in the URL fragment
    KREA2_SKIP_LAUNCH           start everything, then stop before serving
    KREA2_MIRROR_USER           the Hugging Face account the mirrors live under
    KREA2_NO_MIRROR             1 downloads from upstream, never the mirror
    KREA2_CATALOG_FILE          read the catalogue from this file, not the server
...
if SHOWCASE_BASE_URL and not SHOWCASE_BASE_URL.lower().startswith("https://"):
    SHOWCASE_URL_WARNING = (
        "KREA2_SHOWCASE_URL is not an https:// URL — ignoring it. "
...
# KREA2_UI_REQUIRE_TOKEN=1 to put the gate back. See web/api.py.
UI_REQUIRE_TOKEN = bool(os.environ.get("KREA2_UI_REQUIRE_TOKEN"))
...
SKIP_LAUNCH = bool(os.environ.get("KREA2_SKIP_LAUNCH"))

# The Hugging Face account the weight mirrors live under, and the escape
# hatch that ignores them. KREA2_NO_MIRROR=1 goes straight to upstream —
...
CATALOG_FILE_ENV = "KREA2_CATALOG_FILE"
...
def catalog_file() -> str | None:
    ...
    """
    return os.environ.get(CATALOG_FILE_ENV)
...
def ensure_dirs() -> None:
    ...
    is what used to put a /workspace/krea2 tree on any machine that
