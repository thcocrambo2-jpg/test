...
```powershell
$env:KREA2_LICENSE_KEY="<key>"; python app.py
```

bash (WSL or a pod):

```bash
KREA2_LICENSE_KEY=<key> python3 app.py
...
| --- | --- | --- | --- |
| `KREA2_LICENSE_KEY` | none — **required** | The customer key. Takes a seat and decides which tabs exist. Unset, the app stops before any expensive work. | customer |
| `KREA2_NODE_TAG` | none — **required** | The deployment id the key checks in against. The licence API is `https://<tag>.vercel.app`, assembled from this. | customer |
| `KREA2_BASE_DIR` | `/workspace/krea2` (`.dryrun/` under `scripts/dryrun.py`) | Where the ComfyUI install, model weights, generated images and logs live. | customer, developer |
| `KREA2_LICENSE_GRACE` | `1800` (seconds) | How long the app keeps running when the licence server is unreachable. Long enough not to kill a video render mid-way, short enough that a cut-off pod does not run indefinitely. | operator |
| `KREA2_KEEP_MODELS_LOADED` | unset | Set to anything and models are *not* unloaded between swaps. Off by default; see [Model swapping](architecture/model-swapping.md). | developer |
| `KREA2_SAGE_ATTENTION` | `1` | `0`, `false`, `no` or `off` runs ComfyUI with PyTorch attention instead of SageAttention. The first thing to try if a tab's output looks wrong on a card where it did not. | customer |
| `KREA2_WAN_PARALLEL` | unset | Set to anything and video jobs get their own ComfyUI instance on port 8189, so a quick image never queues behind a long render. Only has an effect when the `wan_i2v` feature is on. | operator |
| `KREA2_MAIN_RESERVE_VRAM` | `26` (GB) | Parallel mode only: VRAM the image instance leaves free for the video one. | operator |
| `KREA2_WAN_RESERVE_VRAM` | `22` (GB) | Parallel mode only: VRAM the video instance leaves free for the image one. | operator |
| `KREA2_SHOWCASE_URL` | empty | Public bucket serving the pricing page's screenshots. Empty is a supported state: the section still renders, with a placeholder tile per missing picture. | operator |
| `KREA2_UI_REQUIRE_TOKEN` | unset | Set to anything and the UI demands the access token from the URL fragment. Without it the UI is open to anyone with the link. | operator |
| `KREA2_SKIP_LAUNCH` | unset | Do everything a normal start does — licence, catalogue, setup, downloads, ComfyUI — then stop instead of serving. What a pod that only warms its volume runs. | operator |
| `KREA2_MIRROR_USER` | `thcocrambo2` | The Hugging Face account the weight mirrors live under. | developer |
| `KREA2_NO_MIRROR` | unset | Set to anything and every download goes straight upstream, never to the mirror. For debugging a suspected bad mirror without editing anything. | developer |
| `KREA2_CATALOG_FILE` | unset | Read the model and LoRA catalogue from this JSON file instead of the licence server. What makes a dry run and every check work offline. | developer |
...
- **`KREA2_NODE_TAG` must be one DNS label** — 8 to 63 characters, lower
...
- **`KREA2_BASE_DIR` is expanded and resolved to an absolute path.** A
...
- **`KREA2_SHOWCASE_URL` must be `https://`,** and any query or fragment is
...
| --- | --- | --- |
| `KREA2_LICENSE_KEY` | none — **required** | As above. |
| `KREA2_NODE_TAG` | none — **required** | As above. |
| `KREA2_IMAGE` | `krea2:latest` | A published tag to run instead of a locally built image. |
| `KREA2_USE_BAKED_COMFY` | `1` | `0` ignores the ComfyUI baked into the image; the app clones and pip-installs its own. |
| `KREA2_START_SOURCE` | `baked` | `api` fetches the start script from the licence server, so a fix there applies without a new image. Needs `KREA2_NODE_TAG`. |
| `KREA2_DEV_SOURCE` | unset | Set to `/src` by `docker-compose.dev.yml`; flips the entrypoint to run mounted source instead of the published binary. |
| `KREA2_DEV_IMAGE` | `krea2:dev` | Dev compose only. |
...
| `KREA2_ADMIN_TOKEN` | `build.sh`, `build.ps1` | The deployment's `ADMIN_TOKEN`, which guards `/v1/admin/*`. Exported by `make` from `license-validator/.env`. |
| `KREA2_BUILD_CHANNEL` | `build.sh`, `build.ps1` | Which channel to point this build at. Default `stable`; set something else to upload a build without shipping it to customers. |
...
| `R2_SHOWCASE_BUCKET` | `scripts/r2_upload_showcase.py` | The **public** bucket that serves `KREA2_SHOWCASE_URL`. |
...
| `ADMIN_TOKEN` | admin routes, publishing | Guards `/v1/admin/*`. Unset, those routes 404. `make` exports it as `KREA2_ADMIN_TOKEN`. |
