...
```
Weights → <KREA2_BASE_DIR>/models · images → <KREA2_BASE_DIR>/output
Features — ...
...
- The **Weights** line says where everything will land. If that is not
  where you meant, `KREA2_BASE_DIR` is not set the way you think.
...
- The **Catalogue** line names the source it used: the licence server, the
  cached `.catalog.json`, or a file named by `KREA2_CATALOG_FILE`.
...
| --- | --- |
| Exits within seconds, nothing downloads | `KREA2_LICENSE_KEY` or `KREA2_NODE_TAG` is unset or malformed. The seat is taken before any expensive work, on purpose. The tag has to be one bare DNS label — see [Configuration](configuration.md). |
| Weights land somewhere unexpected | `KREA2_BASE_DIR` is unset, so the pod default `/workspace/krea2` applied — which on Windows resolves to `C:\workspace\krea2`. |
...
| A tab's output looks wrong on a card where it did not | Set `KREA2_SAGE_ATTENTION=0` and try again. SageAttention is approximate and applies to every tab. |
...
| Symptom | Cause and fix |
| --- | --- |
| `WinError 193` from `cloudflared` | A Linux `cloudflared` is cached where the Windows one belongs. Delete `cloudflared.exe` under `KREA2_BASE_DIR` and restart: `serve.RELEASES` picks the asset by `sys.platform`, and `scripts/windows_start.ps1` verifies a sha256 before putting one there. |
...
ComfyUI's own output goes to `comfyui.log` under `KREA2_BASE_DIR`, and the
